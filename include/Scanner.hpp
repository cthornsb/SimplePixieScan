...
class extTree : public TTree {
  ...
  public:
	/** Named constructor.
	  *	@param name_ Name of the underlying TTree.
	  * @param title_ Title of the underlying TTree.
	  */
	...
	/** Set drawing variables to use for drawing from the tree on the next call to SafeFill()
	  * @param expr_ Root TFormula expression to draw from the TTree
	  * @param gate_ Root selection string to use for plotting
	  * @param opt_ Root drawing option
	  */
	void SafeDraw(const std::string &expr_, const std::string &gate_="", const std::string &opt_="");

	/** Use current drawing variables and draw from the tree on the next call to SafeFill()
	  */
	void SafeDraw();
	
	/** Fill the tree and draw a histogram using TTree::Draw() if requested
	  * @param doFill Flag indicating that the tree will be filled
	  * @return The return value from TTree::Fill() or -1 in the event that the tree fill is not requested
	  */
	int SafeFill(const bool &doFill=true);
};
...
class simpleUnpacker : public Unpacker {
  public:
	...
	/** Get a pointer to the statistics output tree
	  */
...
class simpleScanner : public ScanInterface {
  public:
	...
	/** ExtraCommands is used to send command strings to classes derived
	  ...
	  * @param cmd_ The command to interpret.
	  * @param arg_ Vector or arguments to the user command.
	...
	/** CmdHelp is used to allow a derived class to print a help statement about
	  ...
	  * @param prefix_ String to append at the start of any output. Not used by default.
	  * @return Nothing.
	  */
	...
	/** Initialize the map file, the config file, the processor handler, 
	  ...
	  * @param prefix_ String to append to the beginning of system output.
	...
	/** Initialize the root output. 
	  * @param fname_     Filename of the output root file. 
	  * @param overwrite_ Set to true if the user wishes to overwrite the output file. 
	...
	/** Receive various status notifications from the scan.
	  * @param code_ The notification code passed from ScanInterface methods.
	  * @return Nothing.
	  */
	...
	/** Add a channel event to the deque of events to send to the processors.
	  ...
	  * @param event_ The raw XiaData to add.
